...
import type { LoginRequest, RegisterRequest, RenewTokenRequest, UserDto } from '$lib/types/auth';
...
export const AuthService = {
	...
	register: async (data: RegisterRequest, customFetch?: typeof fetch) => {
		return await api.post<UserDto>('auth/register', data, undefined, customFetch);
	},

	renewToken: async (refreshToken: string, customFetch?: typeof fetch) => {
		const payload: RenewTokenRequest = { refreshToken };
		return await api.post<UserDto>('auth/renewToken', payload, undefined, customFetch);
	},
